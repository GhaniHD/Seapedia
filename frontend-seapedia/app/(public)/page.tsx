        ...
        <div className="grid grid-cols-4 gap-3 sm:grid-cols-8">
          {categories.map((c) => (
            <Link
              key={c.label}
              href="/products"
              className="flex flex-col items-center gap-2 rounded-xl border border-sand-200 bg-white p-3 text-center transition-shadow hover:shadow-md"
            >
      ...
      <section className="mx-auto max-w-6xl px-5 py-6">
        ...
        {products.length > 0 ? (
          ...
          <div className="rounded-b-xl border border-t-0 border-sand-200 bg-white p-8 text-center text-sm text-ink/50">
            ...
            <Link href="/products" className="underline">Lihat katalog lengkap</Link>.
          </div>
        )}
      </section>
